...
const AddTechnicalExecutive = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      await set(ref(db, `settings/technical_executives/${key}`), { name: nm, createdAt: Date.now() })
      setStatus('Saved')
      setName('')
    } catch (err) {
  ...
  return (
    <div className="max-w-xl mx-auto">
      <h3 className="text-2xl font-bold text-gray-800 mb-4">Add Technical Executive</h3>
      <form onSubmit={onSubmit} className="space-y-4 bg-white p-6 rounded-lg border border-gray-200">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Name</label>
          <input value={name} onChange={(e)=>setName(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2" placeholder="Full Name" />
        </div>
...
export default AddTechnicalExecutive
