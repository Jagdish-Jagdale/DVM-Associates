...
const MainLayout = ({ onLogout }) => {
  ...
  const sections = useMemo(
    () => [
      { name: 'Dashboard', path: '/admin/dashboard', icon: <MdSpaceDashboard className="text-lg md:text-xl" /> },
      { name: 'Excel', path: '/admin/excel', icon: <FaFileExcel className="text-lg md:text-xl" /> },
      { name: 'Pending', path: '/admin/pending', icon: <FiClock className="text-lg md:text-xl" /> },
    ],
    []
  )
      ...
      <div className="flex flex-1 min-h-0">
        ...
        >
          <div className="h-20 px-4 border-b border-gray-200 bg-gradient-to-b from-white to-gray-50 flex items-center justify-between">
            <button type="button" onClick={() => { navigate('/admin/dashboard'); setSidebarOpen(false) }}>
        ...
        <div className="hidden md:flex w-64 min-w-[16rem] shrink-0 bg-white border-r border-gray-200 flex-col h-screen sticky top-0 overflow-y-auto shadow-sm">
          ...
          {/* Navigation Menu (directly below header divider) */}
          ...
          {/* Sidebar Footer with Logout (pinned bottom) */}
          <div className="mt-auto py-3 px-3 border-t border-gray-200 bg-gray-50">
            <div className="flex flex-col items-center gap-2 md:gap-3">
              <button
                type="button"
